...
class NeedleDetectionThread(QtCore.QThread):
    def __init__(self):
        QtCore.QThread.__init__(self)
        self.running = True
        self.needleDetected = False

    def run(self):               
        while self.running:
            time.sleep(0.00001)
            if (gpio.input(18) == True and self.needleDetected == False ):
                self.needleDetected = True
...
class OriginTouchScreen(QWidget):
    def __init__(self):
        ...
        self.serialThreadMKS = SerialThreadMKS("/dev/AvayUSB", 250000)   # Start serial thread
    ...
    def DefineGlobalVariables(self):
        ...
        self.ratingFrame = QFrame()
        self.calibrationFrame = QFrame()
    ...
    def CreateCalibrationPage(self):
        ...
        atriumMeasuredHBox = QHBoxLayout()
        ...
        atriumExistingHBox = QHBoxLayout()
        ...
        ventricleMeasuredHBox = QHBoxLayout()
        ...
        ventricleExistingHBox = QHBoxLayout()
        ...
        needleDetectionVBox = QVBoxLayout()
    ...
    def UpdateTouchScreen(self,b):
        ...
        if(self.mountedDevice == ""):
            ...
            msgBox.setWindowFlags(QtCore.Qt.WindowStaysOnTopHint| QtCore.Qt.WindowCloseButtonHint)
    ...
    def CameraStatus(self, b1,b2,b3,b4):
        if("ON" in b1.text()):
            ...
            self.cameraThread.recording = True
            ...
            self.cameraThread.recording = False
            b1.setStyleSheet("QRadioButton{font: 15pt Ubuntu Mono; border-radius: 10px; font-weight:bold; background-color: #339999;} QRadioButton::indicator { width: 0px; height: 0px;};")
            b2.setStyleSheet("QRadioButton{font: 15pt Ubuntu Mono; border-radius: 10px; font-weight:bold; background-color: #AAAAAA;} QRadioButton::indicator { width: 0px; height: 0px;};")
    ...
    def StartPrint(self):
        ...
        #self.cameraThread = CameraThread()   # Start thread
        #self.cameraThread.start()
        self.cameraThread.gcodeName = self.gCodeFile.split('/')[-1].split('.')[0]
        ...
        if(self.dataLog == True):
            ...
            nameAvailable = False
            newName = self.cameraThread.gcodeName
            i = 1
            while nameAvailable == False:
                ...
                for l in ls:
                    if(newName == l):
                        nameAvailable = False
                        newName = self.cameraThread.gcodeName + "_" + str(i)
                        break;
                i = i+1
                        
            self.cameraThread.directory = "/home/pi/DataLog/" + newName
            os.mkdir(self.cameraThread.directory)
            logFile = open(self.cameraThread.directory + "/log.txt", 'w')
        ...
        self.cameraThread.cameraNeeded = True
        self.cameraThread.showCamera = True
        self.cameraThread.firstTime = True
        self.cameraThread.firstTimePreview = False
        self.startTime = time.time()
        
        self.stack.setCurrentIndex(10)
        self.gCodeFileLabel.setText(self.cameraThread.gcodeName)
        
        self.serialThreadMKS.serialMKS = serial.Serial(self.serialThreadMKS.portname, self.serialThreadMKS.baudrate,timeout=0.2)
        self.serialThreadMKS.buffer = 1;
        self.nameValue.setText(self.cameraThread.gcodeName)
    ...
    def ShowCamera(self,b):
        self.cameraThread.showCamera = not self.cameraThread.showCamera;
        self.cameraThread.firstTimePreview = True
    ...
    def Reconnect(self):
        ...
        self.serialThreadMKS.running=False
        time.sleep(1)
        self.serialThreadMKS = SerialThreadMKS("/dev/AvayUSB", 250000)   # Start serial thread
        self.serialThreadMKS.start()
    ...
    def StopPrint(self,b,b1):
        ...
        self.printStarted = False
        self.cameraThread.cameraNeeded = False
        self.serialThreadMKS.txq = []
        self.serialThreadMKS.serialMKS.write("M112;\n".encode())
        self.serialThreadMKS.running = False
        time.sleep(1)
